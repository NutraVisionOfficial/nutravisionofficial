...
export function MilestoneTracker() {
  const { startingWeight, currentWeight, goalWeight, goalTimeframeMonths, startDate } = mockUser;

  const totalLoss = startingWeight - goalWeight;
  const currentLoss = startingWeight - currentWeight;
  const progress = Math.min(100, Math.round((currentLoss / totalLoss) * 100));
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-6 animate-fade-in">
      ...
      <p className="text-sm text-muted-foreground mb-5">
        {monthsElapsed} months in · {monthsRemaining} months to go
      </p>
            ...
            <div key={m.label} className="text-center">
              <div
                className={`w-3 h-3 rounded-full mx-auto mb-1 ${
                  m.reached ? "bg-primary" : "bg-border"
                }`}
              />
      ...
      <div className="flex items-center gap-2 text-sm">
        ...
        <span className="ml-auto text-card-foreground font-semibold">
