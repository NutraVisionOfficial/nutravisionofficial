...
export function WeeklyChart({ logs }: WeeklyChartProps) {
  const data = logs.map((l) => ({
    ...
    weight: Number(l.current_weight) || 0,
  }));
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-6 animate-fade-in">
      ...
      <div className="flex gap-6 mt-4 text-xs text-muted-foreground">
        <div className="flex items-center gap-2"><div className="w-3 h-0.5 bg-primary rounded" /> Calories</div>
        <div className="flex items-center gap-2"><div className="w-3 h-0.5 bg-accent rounded" style={{ borderStyle: "dashed" }} /> Weight (kg)</div>
      </div>
    </div>
  );
}
