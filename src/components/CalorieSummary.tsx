interface CalorieSummaryProps {
  todayLog: { total_calories: number; protein: number; carbs: number; fats: number } | null | undefined;
  target: number;
}

export function CalorieSummary({ todayLog, target }: CalorieSummaryProps) {
    ...
    <div className="rounded-xl border border-border bg-card p-6 animate-fade-in">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex-1 space-y-3">
          <MacroBar label="Protein" value={todayLog?.protein ?? 0} max={180} unit="g" />
          <MacroBar label="Carbs" value={todayLog?.carbs ?? 0} max={300} unit="g" />
          <MacroBar label="Fats" value={todayLog?.fats ?? 0} max={80} unit="g" />
